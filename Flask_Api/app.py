...
RF_MODEL_FEATURES = ['calories', 'protein', 'carbohydrate', 'fat', 'sugar', 'sodium', 'saturated_fat', 'gi', 'fiber','potassium']
...
def predict_food_types_for_dataframe(df: pd.DataFrame):
    ...
    numeric_cols = ['calories', 'protein', 'carbohydrate', 'fat', 'sugar', 'sodium', 'saturated_fat', 'gi', 'fiber','potassium']
    ...
    try:
        ...
        print(f" ปัญหาในการ preprocess (imputer/scaler): {e}")
        return np.array([]), pd.Index([])

    if X_scaled.shape[0] == 0:
        print(" ไม่มีข้อมูลเหลือหลัง preprocess")
        return np.array([]), pd.Index([])
...
def predict_food_type(user_data_dict): 
    try:
        ...
        print(f"✅ ประเภทอาหารที่โมเดลผู้ใช้ทำนาย (code): {predicted_food_type_code}")
...
def get_food_details():
    ...
    try:
        query = """
            SELECT food_id, food_name, calories, protein, fat, carbohydrate, sugar, sodium, image_url, amount, category, fiber, potassium
            FROM food_menu 
            WHERE food_id = %s
        """
        ...
        if food_details_row:
            ...
            numeric_fields = ['calories', 'protein', 'fat', 'carbohydrate', 'sugar', 'sodium','fiber', 'potassium']
